  ...
  return this.http.get<any>(url).pipe(
    ...
    switchMap((contactsNoId: any[]) =>
      this.http.post<Contact[]>(`${this.apiUrl}/bulk`, contactsNoId).pipe(
        // שמירה מהשרת ללוקאל (קיבלנו רשומות עם ID אמיתי)
...
async syncToServerDB(): Promise<void> {
  ...
  // משיכה מהשרת: עכשיו לכל החדשים יש Id אמיתי
  const serverContacts = await firstValueFrom(this.http.get<any[]>(this.apiUrl));

  // מחיקה מלאה של הלוקאל והכנסה מהשרת ללוקאל (רענון מלא)
  await firstValueFrom(this.dbService.clear('contact'));
  
  // הכנסת כל הנתונים מהשרת (עם ID אמיתיים) ללוקאל
